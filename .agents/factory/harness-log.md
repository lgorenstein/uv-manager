...
## 2026-09-13 — maintainer request: Step 6 had grown 3.9x by answering one question in four places
`decision=applied commit=faf1f9a target=.agents/skills/uvm-plan/SKILL.md`
- **Rationale:** raised by the maintainer on bloat, not by a `META.md` finding, and measured before acting: Step 6 went 23 → 89 lines across eight cycles, `uvm-plan` 206 → 295 monotonically, and `.agents/` stands at 4606 lines added against 214 deleted excluding this append-only file. The diagnosis that mattered is not the count — every one of those eight decisions was individually correct — but that "how do I read this gate's status" had six answers (green-inert, green-legitimate, red-on-post-condition, red-on-itself, red-on-dependency, can't-tell) scattered over four paragraphs with a shell-trap catalogue spliced between them, so each cycle's finding landed as another sentence in a paragraph already eight revisions deep. Split by `AGENTS.md`'s own rule — tables are reference, prose is reasoning: the four readings became one ordered test, the catalogue a table in `templates/TECH.md`, which already owns gate authoring via `3ce4978` and `c70d818` and is template-only (both landed `spec/*/TECH.md` drop the field reference when copying, so it does not duplicate per cycle). **Rejected unifying it with `review-rubric.md`'s trap list**, which serves the blind reviewer who gets the rubric by path and never sees the template — the same audience question `3ce4978` answered the same way; the `zsh` entry stays duplicated across that boundary deliberately, and a future run should not collapse it. Step 6 89 → 70, `uvm-plan` 295 → 276, the first net reduction in that file across nineteen commits. Re-verified the moved claims rather than trusting the move: `sh -c 'set -e; ! true; echo REACHED'` exits 0 where `! true` alone exits 1, and `zsh` yields one field where `bash` yields two. **The generalizable half is the destination, not the cleanup:** a finding that adds a *fact* now has a table row to land in, where before it cost an edit to load-bearing prose.

## 2026-09-13 — maintainer request: the loop had no answer to unbounded growth
`decision=applied commit=c4b6715 target=.agents/skills/uvm-harness/SKILL.md`
- **Rationale:** the maintainer raised a bounded harness size and was unsure of it; **the size cap was argued against and not built.** A cap makes deletion compete with correctness — an agent at the ceiling facing a correct finding must delete something correct to fit it — and it is met by densifying prose, which costs the next reader more than the lines saved; `8d888a0` already refused exactly that trade when compressing 63 ledger entries would have deleted the anti-thrash memory they exist to carry. It also prices the wrong thing: `harness-log.md` is the largest file in the factory at 746 lines and costs a reader nothing, being scanned once per run, while Step 6 at 89 lines was expensive because it was reasoning an agent had to hold while working. What landed instead is a classification at Step 3: a **fact** (a trap, a command's output shape, a portability quirk) goes to a lookup structure and its list may grow without bound; a **judgment** lives in prose and must name the existing judgment it merges with, supersedes or sits beside. Applied to the case that produced it, `e935214`, `7b0de8b` and `d83498c` are three answers to one question and the rule would have made them meet three cycles before `faf1f9a` had to. **Known weakness, stated rather than hidden:** the fact/judgment split is itself a judgment and an agent could route down the cheap path to avoid the work — the classification is made at Step 3 in front of the human, which is where that gets caught. Corrected this step's own figures in the same commit: they asserted 63 applied against **0** rejected and that Safety §6's previously-rejected branch had never been written to, both false since F11 the day before, and rephrased so they date themselves rather than needing re-measurement every run.
